  ...
  <b-form class="container" @submit="handleMultipleChoiceSubmit">
    <b-form-group v-if="node.typeData.options.multipleChoice.multiAnswer">
      <b-form-checkbox-group v-model="userSelectedCheckbox">
        <user-choice-row
          v-for="userChoiceRow in node.typeData.options.multipleChoice.checkboxArray"
    ...
    <b-form-group v-else-if="!node.typeData.options.multipleChoice.multiAnswer">
      <b-form-radio-group v-model="userSelectedRadio">
        <user-choice-row
          v-for="userChoiceRow in node.typeData.options.multipleChoice.radioArray"
          :key="userChoiceRow.id"
          :item="userChoiceRow"
          :isCheckBox="node.typeData.options.multipleChoice.multiAnswer"
          :hasImage="node.typeData.options.multipleChoice.useImages"
  ...
  computed: {
    ...
    getPreSelectedRadioValue() {
      if (!this.node.typeData.options.multipleChoice.multiAnswer) {
        if (
          this.node.typeData.options.multipleChoice.selectedRadioArray.length > 0
        ) {
          var matchingID = this.node.typeData.options.multipleChoice
            .selectedRadioArray[0]
    ...
    getPreSelectedCheckBoxValue() {
      if (this.node.typeData.options.multipleChoice.multiAnswer) {
        var selectedCheckBoxIDs = this.node.typeData.options.multipleChoice
