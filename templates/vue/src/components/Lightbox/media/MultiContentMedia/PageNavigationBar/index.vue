...
export default {
  ...
  mounted() {
    ...
    for (const ref of this.$parent.$refs.rowRefs) {
  ...
  methods: {
    ...
    nextTab() {
      const rowOrder = [13048, 13050, 13052] // TODO: Get actual row values
      const nextTabIndex = rowOrder.indexOf(this.active)
      if (nextTabIndex >= 0 && nextTabIndex < rowOrder.length - 1) {
        return rowOrder[nextTabIndex + 1]
      }
      return this.active
    },
    scrollToRef(nodeId) {
      if (nodeId) {
        this.active = nodeId
        this.$nextTick(() => {
          let el = this.$parent.$parent.$refs.rowRefs.find(
            ref => ref.id === `row-${nodeId}`
          )
