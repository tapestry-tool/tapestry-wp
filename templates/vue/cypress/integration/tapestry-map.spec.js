...
describe("Map Tapestry", () => {
    ...
    it("should be able to add markers to map if and only if coordinates are valid", () => {
        ...
        cy.getByTestId('node-lat-input').clear().type("90")
        cy.getByTestId('node-lng-input').clear().type("180")
        cy.getByTestId("submit-node-modal").should('not.be.disabled')
        cy.getByTestId('node-lat-input').clear().type("-50")
        cy.getByTestId('node-lng-input').clear().type("-150")
        cy.getByTestId("submit-node-modal").should('not.be.disabled')
        cy.submitModal()
    ...
    it('should see map markers and be able to click to view nodes even if logged out', () => {
        ...
        cy.get('.leaflet-popup-content > div > button').should('be.visible')
        cy.get('.leaflet-popup-content > div > button').click({force:true})
    ...
    it("can add new nodes", () => {
        ...
        cy.get('.nodes-list').contains(newNode.title).should("be.visible")
    })
})