        ...
        */
        ...
        function tapestry_enqueue_vue_app()
        {
            ...
            if ('tapestry' == get_post_type($post) && !post_password_required($post)) {
                ...
                $useDev = (defined('TAPESTRY_USE_DEV_MODE') && !empty(TAPESTRY_USE_DEV_MODE)) || isset($_GET['debug']);

                // register the Vue build script.
                $vueUrl = $useDev ? 'http://localhost:8080/dist' : plugin_dir_url(__FILE__).'templates/vue/dist';
                ...
                $iframeMode = array_key_exists('iframe', $_GET) ? 1 : 0;
        ...
        function prefix_title_entity_decode($response)
        {
            ...
            $data['title']['rendered'] = wp_specialchars_decode(html_entity_decode($data['title']['rendered']), $quoteStyle);
            $data['title']['rendered'] = replace_special_apostrophe($data['title']['rendered']);
            $data['content']['rendered'] = wp_specialchars_decode(html_entity_decode($data['content']['rendered']), $quoteStyle);
